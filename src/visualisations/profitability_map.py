...
def profitability_map(df: pd.DataFrame) -> None:
    """Create a geographic scatter plot of profitability across wind farm locations."""
    color_column = config.TARGET_PROFITABILITY_COLUMN
    color_min, color_max = get_colors_range(df, color_column)
    fig = px.scatter_geo(
        df,
        lat="LAT",
        lon="LON",
    ...
    fig.write_html(config.MAP_HTML_PATH)

def get_colors_range(df: pd.DataFrame, column: str) -> tuple[float, float]:
