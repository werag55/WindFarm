...
def prepare_cleaned_dataset(csv_path: str) -> pd.DataFrame:
    ...
    for col in config.CATEGORICAL_COLUMNS:
        if col in df.columns:
            final_columns.extend([c for c in df.columns if c.startswith(f"{col}_")])
...
def _convert_range_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col_name, suffix in config.RANGE_COLUMNS.items():
        ...
        df[f"{base_name}_min_{suffix}"] = df[col_name].apply(lambda x: to_float_range_mean(x)[0])
        df[f"{base_name}_max_{suffix}"] = df[col_name].apply(lambda x: to_float_range_mean(x)[1])
        df[f"{base_name}_mean_{suffix}"] = df[col_name].apply(lambda x: to_float_range_mean(x)[2])

    df["installed_capacity_MW"] = df["installed_capacity_MW"] \
        .apply(lambda x: to_float_range_mean(x)[2] if pd.notna(x) else np.nan)
    return df
